// ==UserScript==
// @name bittrex USD Enhancer
// @description Adds USD conversion all over the place
// @author toneffectory
// @orig-autor Copyright (c) 2017 Nirvana Tikku (MIT license, chrome addon)
// @version 1.0
...
Enhancer.updatePrices = function updatePrices(){
  if(Enhancer.pageType === 'markets' || Enhancer.pageType === 'history' || Enhancer.marketType === 'eth'){
...
Enhancer.getMarketType = function getMarketType(mktType){
  ...
  return Enhancer.marketType = (mkt === 'btc' ? 'btc' : (mkt === 'eth' ? 'eth' : (mkt === 'usdt' ? 'usdt' : '')));
};
